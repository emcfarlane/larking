...
/*
Package server implements the building blocks to setup end-to-end gRPC benchmarks.
*/
package server
...
func StartServer(info ServerInfo, _ ...grpc.ServerOption) func() {
	m, err := larking.NewMux()
	if err != nil {
		logger.Fatalf("failed to StartServer, NewMux failed: %v", err)
	}

	//s := grpc.NewServer(opts...)
	switch info.Type {
	case "protobuf":
		testpb.RegisterBenchmarkServiceServer(m, &testServer{})
	case "bytebuf":
		respSize, ok := info.Metadata.(int32)
		if !ok {
			logger.Fatalf("failed to StartServer, invalid metadata: %v, for Type: %v", info.Metadata, info.Type)
		}
		testpb.RegisterBenchmarkServiceServer(m, &byteBufServer{respSize: respSize})
	default:
		logger.Fatalf("failed to StartServer, unknown Type: %v", info.Type)
	}

	s, err := larking.NewServer(m)
	if err != nil {
		logger.Fatalf("failed to StartServer, NewServer failed: %v", err)
	}
