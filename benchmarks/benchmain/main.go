...
func makeClient(bf stats.Features) (testpb.BenchmarkServiceClient, func()) {
	...
	lis = nw.Listener(lis)
	stopper := server.StartServer(server.ServerInfo{Type: "protobuf", Listener: lis})
	target := "localhost:5051"
	conn := benchmark.NewClientConn(target /* target *IS* used */, opts...)
